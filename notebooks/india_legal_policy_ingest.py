...
# Secrets: create scope `bharatbricks-hack` in Databricks (User Settings → Secrets) or set env vars locally.
import os

def _read_secret(env_name: str, scope_key: str = None):
    """Databricks secret scope `bharatbricks-hack`, else upper-case env var."""
    try:
        return dbutils.secrets.get(scope="bharatbricks-hack", key=scope_key or env_name.lower())  # noqa: F821
    except Exception:
...
if not DATAGOV_API_KEY:
    print('⚠️  DATAGOV_API_KEY is empty — set env DATAGOV_API_KEY or secret bharatbricks-hack/datagov_api_key')
else:
...
# MAGIC Runs one minimal `chat/completions` call. Requires secret `bharatbricks-hack` / `sarvam_api_key` or env `SARVAM_API_KEY`. [Dashboard](https://dashboard.sarvam.ai)
# MAGIC
...
if not _sk:
    print('⚠️  No Sarvam key — set SARVAM_API_KEY or secret bharatbricks-hack/sarvam_api_key')
else:
